...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 2) {
            db.execSQL("ALTER TABLE " + TABLE_ITEMS +
                       " ADD COLUMN " + COL_LATITUDE  + " REAL DEFAULT 0.0");
            db.execSQL("ALTER TABLE " + TABLE_ITEMS +
                       " ADD COLUMN " + COL_LONGITUDE + " REAL DEFAULT 0.0");
        }
    }
    ...
    public long insertItem(Item item) {
        ...
        cv.put(COL_LATITUDE,      item.getLatitude());   // NEW
        cv.put(COL_LONGITUDE,     item.getLongitude());  // NEW
    ...
    /** Returns only items that have valid GPS coordinates (for the map). */
    public List<Item> getItemsWithLocation() {
        List<Item> items = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.query(TABLE_ITEMS, null,
            "(" + COL_LATITUDE + " != 0.0 OR " + COL_LONGITUDE + " != 0.0)",
            null, null, null, COL_ID + " DESC");
        if (cursor.moveToFirst()) {
            do { items.add(cursorToItem(cursor)); } while (cursor.moveToNext());
        }
        cursor.close();
    ...
    private Item cursorToItem(Cursor cursor) {
        return new Item(
            ...
            cursor.getDouble(cursor.getColumnIndexOrThrow(COL_LATITUDE)),   // NEW
            cursor.getDouble(cursor.getColumnIndexOrThrow(COL_LONGITUDE))   // NEW
        );
    }
}
